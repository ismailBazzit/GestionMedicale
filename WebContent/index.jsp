...
<body>
	...
	<table class="table table-striped table-bordered " style="width:75%;margin-left:285px">
		<thead class="table-info ">
			<div class="table" >
				<div class="table_header" style="display: flex;justify-content: space-between;align-items: center;padding: 20px;">
					...
					<h2 style="margin-left:70px">Liste des Médecines</h2>
					<div>
						<a href ="<%=request.getContextPath()%>/newMedcin" class="add_new" style="font-size:22px">+ Ajouter</a>
					</div>
				</div>
			</div>
			<tr>
		...
		<tbody>
		<c:forEach var="medcin" items="${list}">
			<tr>
			<td>${medcin.getId()}</td>
			<td>${medcin.getEmail()}</td>
			<td>${medcin.getNomComplet()}</td>
			<td>${medcin.getSpecialite()}</td>
			<td>${medcin.getTel()}</td>
			<td>
			  <a href ="<%=request.getContextPath()%>/editConsultation?id=<c:out value='${medcin.getId()}' />" 
				class="btn btn-warning"><img src="edit.png" style="width:20px";height="20px";></a>
				<a href ="<%=request.getContextPath()%>/deleteMedcin?idM=<c:out value='${medcin.getId()}' />" 
